...
|----------|------------|----------------------|
| [`workflow-publish-with-pinned-action.yml`](workflow-publish-with-pinned-action.yml) | Caller workflow: checkout + **pinned** `uses: sonupreetam/gemara-publish-oci@<tag-or-sha>` | Copy into a **content repo** under `.github/workflows/`; replace `file` / `repository` / triggers. |
...
1. **Land or tag** this branch (or merge to `main`), tag **`v0.1.0`** when ready, update the workflow example to that ref.  
2. **Image publish** — workflow that builds `Dockerfile.publish.sketch` and pushes `ghcr.io/<owner>/gemara-bundle-publish-tool:<tag>` (name TBD by maintainers).  
