...
On demo branches, the active caller configuration is push-only (`copy-only`) with signing disabled.
This action still supports full trust controls, but authoritative cross-registry verification is
performed by caller repositories when enabled there.

```mermaid
flowchart LR
  caller[caller_repo] --> action[action_composite]
  action --> ghcr[ghcr_image_tag]
  ghcr --> dest[destination_registry_tag]
  dest --> outputs[source_ref_and_destination_ref]
```
