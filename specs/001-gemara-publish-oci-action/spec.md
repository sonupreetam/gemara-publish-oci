...
compatibility mode, plus keyless sign/verify and optional cross-registry promotion with explicit
trust modes.
...
source, signs/verifies source digest, optionally promotes to a destination registry, and returns source/destination
outputs.
...
5. **Optional promotion:** copy source -> destination registry with selected trust mode and destination checks.
