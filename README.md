...
> **Branch `gemara-bundle-publish-replacement`:** This tree is the **SDK bundle publish** development that previously lived under a `gemara-bundle-publish-action/` folder in a local monorepo workspace. The repository’s **`main`** branch still carries the historical **transport-only** [`layout-copy`](https://github.com/sonupreetam/gemara-publish-oci/tree/main) action until maintainers reconcile or merge. Spec paths that mention `gemara-bundle-publish-action/` refer to **this repo root** on this branch.
...
jobs:
  publish:
    ...
    steps:
      ...
      - uses: sonupreetam/gemara-publish-oci@gemara-bundle-publish-replacement
        id: publish
        with:
          registry: ghcr.io
